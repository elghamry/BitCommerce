package sa.biotic.app.components.stepper
...
//                goToStep(i)
    ...
    private fun createLines() {
        ...
        lineView.setBackgroundColor(ContextCompat.getColor(context, R.color.default_step_color))
    ...
    private fun createCompleteLine() {
        ...
        completeLineView.setBackgroundColor(
            ContextCompat.getColor(
                context,
                R.color.selected_step_color
            )
        )
            ...
            if (isInitialStep()) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    TransitionManager.beginDelayedTransition(
                        this,
                        getCompleteLineTransition(checkViews.get(currentStep))
                    )
                } else {
                    checkViews.get(currentStep).markAsFinished()
                    incrementCurrentStep()
        ...
        if (checkViews.get(currentStep).isMarkAsFinished()) {
            ...
            if (isInitialStep()) {
                ...
                constraintSet.connect(
                    completeLineView.id,
                    ConstraintSet.END,
                    guidelines.get(currentStep - 1).id,
                    ConstraintSet.END,
                    0
                )
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    TransitionManager.beginDelayedTransition(
                        this,
                        getCompleteLineReverseTransition(checkViews.get(currentStep))
                    )
                    checkViews.get(currentStep).checkButton()
                } else {
                    checkViews.get(currentStep).checkButton()
    ...
    private fun getCompleteLineTopMargin(): Int {
        val valueInPixels = resources.getDimension(R.dimen.checkview_size).toInt()
    ...
    private fun getCompleteLineTransition(checkView: CheckView): TransitionSet? {
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            TransitionSet()
                .addTransition(ChangeBounds().setDuration(200).addTarget(completeLineView.id).addListener(
    ...
    private fun getCompleteLineReverseTransition(checkView: CheckView): TransitionSet? {
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            TransitionSet()
                .addTransition(ChangeBounds().setDuration(300).addTarget(completeLineView.id).addListener(
                    object : Transition.TransitionListener {
                        override fun onTransitionEnd(p0: Transition?) {
                            //no use
                        }
    ...
    private fun recoverState(currentStep: Int = 0) {
        ...
        if (currentStep > 0) {
            ...
            constraintSet.connect(
                completeLineView.id,
                ConstraintSet.END,
                checkViews.get(currentStep - 1).id,
                ConstraintSet.END,
                5
            )
    ...
    fun goToCheckedStep(step: Int = 0) {
